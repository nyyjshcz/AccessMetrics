...
生产部署使用管理员/访客访问密钥作为应用访问边界，但仍应配置 TLS、真实域名、与外部 HTTPS 访问源完全一致的生产 `APP_BASE_URL`、Docker secret 形式的三把密钥和独立的数据/证据目录。生产扫描 Worker 还必须配置 `EGRESS_PROXY_URL`；代理、容器隔离和实际部署环境属于部署者的外部输入。没有这些条件时，不要把本地服务直接暴露到公网。具体步骤见 [部署说明](docs/ops/deployment.md)。

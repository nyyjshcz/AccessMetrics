...
export async function renderRunReportHtml(
  ...
  return `<!doctype html><html lang="${copy.lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${escapeHtml(copy.title)} - ${escapeHtml(dto.site.name)}</title><style>${reportStyles()}${reportGroupingStyles()}</style></head><body>${body}</body></html>`;
}
...
/**
 * Rule cards use this additional layout in every output. Keeping it separate
 * avoids disturbing the existing, already-tested report stylesheet.
 */
export function reportGroupingStyles() {
  return `
    .issue-card .issue-content{padding:16px 18px}
    .rule-guidance{margin:12px 0;padding:12px;border:1px solid #dfe8ef;border-radius:10px;background:#f8fbfc}
    .rule-guidance h4{margin:0 0 6px;font-size:16px}
    .rule-guidance p{margin:0;color:var(--muted);font-size:13px}
    .rule-guidance dl{display:grid;gap:8px;margin:10px 0 0}
    .rule-guidance dl>div{display:grid;grid-template-columns:150px minmax(0,1fr);gap:12px}
    .rule-guidance dt{font-weight:800;color:#23445d}
    .rule-guidance dd{margin:0;color:#38566c}
    .rule-summary{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin:12px 0}
    .rule-summary>div{padding:10px 12px;border:1px solid #dfe8ef;border-radius:9px;background:#fff}
    .rule-summary span{display:block;color:var(--muted);font-size:12px;font-weight:700}
    .rule-summary strong{display:block;margin-top:2px;font-size:22px;line-height:1.2}
    .rule-page-list{margin-top:12px;border:1px solid #dfe8ef;border-radius:10px;background:#fbfdfe}
    .rule-page-list>summary{padding:11px 13px;cursor:pointer;color:#066d9c;font-size:13px;font-weight:800}
    .rule-page-list>summary::-webkit-details-marker{display:none}
    .rule-page-list[open]>summary{border-bottom:1px solid #e2ebf1}
    .rule-page-list-content{display:grid;gap:9px;padding:10px}
    .rule-page-evidence{border:1px solid #dfe8ef;border-radius:10px;background:#fbfdfe}
    .rule-page-evidence summary{display:flex;justify-content:space-between;gap:14px;padding:12px 14px;cursor:pointer;font-size:13px;font-weight:750}
    .rule-page-evidence summary span:first-child{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .rule-page-evidence summary span:last-child{flex:none;color:var(--muted);font-weight:600}
    .rule-page-evidence-body{padding:0 14px 14px;border-top:1px solid #e2ebf1}
    .rule-page-evidence-body>a{display:inline-block;margin-top:12px;color:#066d9c;font-size:13px;font-weight:700}
    @media(max-width:760px){.issue-card .issue-content{padding:14px}.rule-guidance dl>div{grid-template-columns:1fr;gap:3px}.rule-page-evidence summary{display:grid;gap:3px}.rule-page-evidence summary span:first-child{white-space:normal}}
    @media print{.rule-page-list>summary,.rule-page-evidence>summary{list-style:none}.rule-page-list>summary::-webkit-details-marker,.rule-page-evidence>summary::-webkit-details-marker{display:none}.rule-page-list:not([open])>*:not(summary),.rule-page-evidence:not([open])>*:not(summary){display:block!important}.rule-page-list-content,.rule-page-evidence-body{display:block!important}}
  `;
}

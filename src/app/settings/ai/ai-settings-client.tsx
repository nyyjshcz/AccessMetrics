...
export default function AiSettingsClient({ locale = "zh-CN" }: { locale?: Locale }) {
  ...
  const ui = locale === "en"
    ? {
        ...
        noRateLimit: "Disable 20 requests/min (continue at maximum concurrency)",
        rateLimit: "Enable 20 requests/min policy",
        rateHelp: "This affects new AI batches only; maximum concurrency remains independent. When enabled, requests are throttled to 20/min, suitable for free OpenRouter models.",
    ...
    : {
        ...
        noRateLimit: "不启用 20 请求/分钟（按最大并发持续处理）",
        rateLimit: "启用 20 请求/分钟策略",
        rateHelp: "只影响新建的 AI 批次；最大并发仍独立生效。启用后会按 20 请求/分钟节流，适合 OpenRouter 免费模型。",

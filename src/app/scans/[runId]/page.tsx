...
export default function RunPage({ params }: { params: Promise<{ runId: string }> }) {
  const { runId } = use(params); const [data, setData] = useState<any>(); const [tab, setTab] = useState("overview"); const [violations, setViolations] = useState<any[]>([]); const [reviewRefreshKey, setReviewRefreshKey] = useState(0); const [error, setError] = useState(""); const [message, setMessage] = useState("");
  useEffect(() => { fetch(`/api/runs/${runId}`, { cache: "no-store" }).then(async r => { const d = await r.json(); if (!r.ok) throw new Error(d.error?.message ?? "读取扫描失败"); return d; }).then(setData).catch(e => setError(e.message)); }, [runId]);
  ...
  const s = data.score, incomplete = s.resultNodeCounts?.incomplete ?? 0, violationCount = s.resultNodeCounts?.violation ?? 0; const tabs = [["overview", "概览"], ["violations", `自动问题 (${violationCount})`], ["incomplete", `待判断 (${incomplete})`], ["report", "报告"]];
  return <section><div className="card"><p className="eyebrow">扫描结果</p><h1>{data.run.name}</h1><p className="muted">{data.run.origin} · {data.run.status}</p><div className="tabbar" role="tablist">{tabs.map(([key, label]) => <button type="button" key={key} role="tab" aria-selected={tab === key} className={tab === key ? "active" : "secondary"} onClick={() => setTab(key)}>{label}</button>)}</div>{tab === "overview" && <Overview score={s} data={data} />}{tab === "violations" && <ViolationList items={violations} />}{tab === "incomplete" && <><IncompleteReview runId={runId} refreshKey={reviewRefreshKey} /><AiOverlayCard runId={runId} pages={data.pages ?? []} readOnly={data.run.published === 1} onBatchChange={() => setReviewRefreshKey((value) => value + 1)} /></>}{tab === "report" && <Report data={data} runId={runId} publish={publish} />}{message && <p role="status">{message}</p>}</div></section>;
}
function Overview({ score, data }: any) { return <><div className="score">{score.overall === null ? "无可计算数据" : `${score.overall} / 100`}</div><p>页面 {score.pageCount} · 自动问题 {score.resultNodeCounts?.violation ?? 0} · 待判断 incomplete {score.resultNodeCounts?.incomplete ?? 0}</p><div className="grid">{[["可感知", score.perceivable], ["可操作", score.operable], ["易理解", score.understandable], ["兼容性", score.robust]].map(([n, v]) => <div className="card" key={String(n)}><h2>{n}</h2><div className="score">{v === null ? "N/A" : String(v)}</div></div>)}</div><p className="notice">下一步：查看自动问题，处理待判断项目，然后在报告页预览、导出或发布。</p>{data.run.published && <p className="notice">已发布：该扫描及报告为只读。</p>}</>; }
...
function Report({ data, runId, publish }: any) {
  ...
  return <div>
    ...
    <p>规则 {score.ruleCount ?? 0} · 评分机会 {score.opportunities ?? 0} · 自动通过 {score.automaticPassNodes ?? 0} · 自动失败 {score.automaticFailNodes ?? 0} · 加权失败值 {score.weightedDefects ?? 0}</p>
    <p>最佳实践问题 {score.bestPracticeIssueCount ?? 0} · AAA 问题 {score.aaaIssueCount ?? 0} · 未映射 WCAG {score.unmappedWcagIssueCount ?? 0} · 覆盖受限页 {data.coverage?.limitedPages ?? 0} · frame 错误 {data.coverage?.frameErrors ?? 0}</p>
    <h3>四项原则</h3>

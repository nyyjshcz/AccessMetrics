...
docker save -o $bundle accesscheck-nas:local accesscheck-nas-egress:local caddy:2.10.2-alpine
...
# 停止并移除本项目容器/网络；不要加 -v
sudo docker compose --env-file .env.nas -f compose.nas.yaml down --remove-orphans

# 只核对并删除旧的 AccessCheck 镜像；不要使用全局 system prune
sudo docker image ls --all --filter 'reference=accesscheck-nas*'
sudo docker image ls --all --filter 'reference=accesscheck-nas-egress*'
# 根据上两条命令核对后，按准确的旧 tag 或 image ID 执行 docker image rm
...
确认新版本健康后，删除 NAS 上剩余的旧 AccessCheck 镜像；旧归档在本机保留到验收完成。清理时不要删除 `data`、`private-inputs`、`.secrets` 或 Caddy 命名卷。

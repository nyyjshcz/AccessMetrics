  ...
  try {
    ...
    if (
      Object.keys(body).some(
        (key) => !["studyFreezeId", "sourceExportId", "sourceManifestHash"].includes(key),
      )
    )
    ...
    if (!body.studyFreezeId || !body.sourceExportId || !body.sourceManifestHash)
      throw new AppError(
        "INVALID_INPUT",
        "studyFreezeId/sourceExportId/sourceManifestHash 必填",
        422,
      );
    if ("seed" in body)
      throw new AppError("UNKNOWN_FIELD", "seed 由服务端根据 populationDigest 计算", 400);
    return NextResponse.json(
      createManualReviewBatch(body.studyFreezeId, body.sourceExportId, body.sourceManifestHash),
      { status: 201 },
    );
  } catch (error) {

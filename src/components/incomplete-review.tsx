...
export default function IncompleteReview({
  ...
  const load = useCallback(async (p = page) => { const requestId = ++request.current; const r = await fetch(`/api/runs/${runId}/incomplete?page=${p}&pageSize=20`, { cache: "no-store" }); const d = await r.json(); if (!r.ok) throw new Error(d.error?.message ?? "读取 incomplete 扫描结果失败"); if (!mounted.current || requestId !== request.current) return; setItems(d.items ?? []); setMeta(d.pagination); setState({ manualLocked: Boolean(d.manualLocked), readOnly: Boolean(d.readOnly) }); setSelected((old: any) => d.items?.find((x: any) => x.id === old?.id) ?? d.items?.[0]); }, [runId, page]);
  useEffect(() => {
    mounted.current = true;
    void load().catch(e => { if (mounted.current) setError(e instanceof Error ? e.message : "读取 incomplete 扫描结果失败"); });

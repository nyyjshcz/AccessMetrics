...
pnpm test:all             # 上述依赖、静态检查、hygiene/docs/handoff:check、egress policy、47 个 Vitest 测试、Python 分析、构建与 3 个 E2E 总门
...
在此基础上让打印 HTML/PDF 渲染器复用报告样式模板后，候选/CLI 局部测试与完整质量门再次通过（全量 12 个文件、43 个测试）；缺少或非法样式模板会在渲染前停止，未安装 LibreOffice/Poppler 仍保持原有 fail-closed。

...
export async function GET(
  ...
  try {
    ...
    const totalNodes = Number(totalRow.total ?? 0);
    ...
    const impactByRank: Record<number, string | null> = {
      0: null,
      1: "minor",
      2: "moderate",
      3: "serious",
    ...
    const pageStatsById = new Map(
      pageStatsRows.map((row) => [
        row.page_id,
        {
          ...
          highestImpact: impactByRank[Number(row.highest_impact_rank ?? 0)] ?? null,
        },
      ]),
    );
    const nodeRows = db
    ...
    const grouped = new Map<string, { pageId: string; page: { url: string; title: string | null }; nodes: any[] }>();
    for (const row of nodeRows) {
      let group = grouped.get(row.page_id);
      if (!group) {
        group = { pageId: row.page_id, page: { url: row.canonical_url, title: row.title }, nodes: [] };
    ...
    return NextResponse.json({
      ...
      pagination: {
        ...
        hasMore: offset + nodeRows.length < totalNodes,
      },
    });
  } catch (error) {

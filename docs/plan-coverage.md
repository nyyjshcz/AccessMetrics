...
| 4 | SQLite 数据库和迁移 | `AUTOMATED_COMPLETE` | `migrations/`、数据库检查、备份恢复脚本 | `pnpm db:check`、集成迁移/恢复测试 | 正式数据仍需外部输入 |
...
| 8 | 单页 axe 扫描器 | `AUTOMATED_COMPLETE` | Playwright、四类 axe 结果、frame coverage、失败记录 | fixture 扫描、E2E、集成测试 | 真实采集窗口和许可由负责人提供 |
...
| 12 | API | `AUTOMATED_COMPLETE` | OpenAPI 路由契约、统一错误、分页/筛选/导出/发布 API | `pnpm contract:check`、集成/E2E | 无 |
| 13 | 完整 Web 页面 | `AUTOMATED_COMPLETE` | 首页、扫描进度、结果、问题、研究、报告、reviewer 页面 | Next build、Playwright E2E、axe 页面检查 | 无 |
| 14 | HTML/PDF/CSV/JSON 导出 | `AUTOMATED_COMPLETE` | run/study export、manifest/hash、报告/DOCX/PDF 生成与验证 | 集成导出测试、候选报告测试、`pnpm deliverables:verify` | LibreOffice/Poppler 逐页视觉 QA 需外部环境/人工 |
